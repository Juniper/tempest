...
export KEYSTONE_SERVICE_HOST=${KEYSTONE_SERVICE_HOST:-keystone.openstack.svc.cluster.local}
...
export API_SERVER_IP=${API_SERVER_IP:-172.17.0.1}
...
export HTTP_IMAGE_PATH=${HTTP_IMAGE_PATH:-http://10.204.217.158/images/cirros/cirros-0.3.5-x86_64-disk.img}
...
# SYNC demo project with contrail
sync_demo_project $API_SERVER_IP $ADMIN_USERNAME $ADMIN_PASSWORD
...
create_user $USERNAME $password $TENANT_NAME "admin"
...
create_endpoint $CATALOG_TYPE "http://"${API_SERVER_IP}
...
#IDENTITY
iniset $TEMPEST_CONFIG identity uri "http://keystone.openstack.svc.cluster.local:80/v2.0/"
...
iniset $TEMPEST_CONFIG network-feature-enabled api_extensions allowed-address-pairs,extra_dhcp_opt,security-group,floating_ips,port_security,ipv6,router,quotas,binding,lbaas
...
iniset $TEMPEST_CONFIG identity-feature-enabled api_v2_admin true
...
iniset $TEMPEST_CONFIG sdn "endpoint_type" $ENDPOINT_TYPE
iniset $TEMPEST_CONFIG sdn "catalog_type" $CATALOG_TYPE
